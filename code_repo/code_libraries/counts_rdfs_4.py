...
def compute_distances_PBC_chol_to_acyl_one_ts(chol_array_one_ts, acyl_array_one_ts, box_dim_vector):
    """
    Compute distances from CHOL COMs to acyl COMs (flipped from acyl-to-chol).
    Returns shape [no_chol, no_acyl] where each row is distances from one CHOL to all acyls.
    """
    ...
    for chol_com_index in range(no_chol):
        ...
        for acyl_com_index in range(no_acyl):
            ...
            current_chol_acyl_pbc_distance = radial_distance_pbc_2d(
                current_acyl_com, current_chol_com, box_dim_vector)[0]
            chol_distances_one_com[acyl_com_index] = current_chol_acyl_pbc_distance
            
        # preserve acyl index identity across frames
        all_pbc_distances[chol_com_index] = chol_distances_one_com
...
def compute_distances_PBC_chol_to_acyl_all_ts(chol_array, acyl_array, box_dim_vector):
    """
    Compute distances from CHOL COMs to acyl COMs for all timesteps.
    """
    ...
    for ts in range(no_ts):
        ...
        one_ts_pbc_distances = compute_distances_PBC_chol_to_acyl_one_ts(
            chol_array[ts], acyl_array[ts], box_dim_vector
        )
...
def RDF_calculator_onelist_diff_species(distances_one_atom, dr, cutoff_distance, box_dim_vector):
    
    # sort here, only when needed for RDF shell counting
    sorted_distances = np.sort(distances_one_atom)
    
    radial_distances = np.arange(0, cutoff_distance, dr)
    outer_radius = 0
    inner_radius = 0
    density_global = len(sorted_distances) / (box_dim_vector[0] * box_dim_vector[1])
    hit_list = []
    ...
    for i in range(len(radial_distances)):
        ...
        for j in range(len(sorted_distances)):
            if (sorted_distances[j] > inner_radius) and (sorted_distances[j] < outer_radius):
                hits += 1
...
def RDF_over_all_atoms_one_frame_diff_species(pbc_sorted_distances, dr, cutoff_distance, box_dim_vector):
    ...
    for i in range(len(pbc_sorted_distances)):
        ...
        # Skip if this distance array is empty
        if len(pbc_sorted_distances[i]) == 0:
            continue
    ...
    # If no valid data, return NaN array
    if len(unweighted_rdfs) == 0:
        return np.full(len(np.arange(0, cutoff_distance, dr)), np.nan)
...
def RDF_over_all_frames_diff_species(vec_timesteps, dr, cutoff_distance, box_dim_vector):
    ...
    for i in range(len(vec_timesteps)):
        ...
        if len(vec_timesteps[i]) == 0:
            ...
            if not np.all(np.isnan(rdf_one_timestep)):
                time_unaveraged_RDFS.append(rdf_one_timestep)
    
    if len(time_unaveraged_RDFS) == 0:
        return np.full(len(np.arange(0, cutoff_distance, dr)), np.nan)
    ...
    for j in range(len(time_unaveraged_RDFS[0])):
        ...
        for k in range(len(time_unaveraged_RDFS)):
            ...
            if not np.isnan(time_unaveraged_RDFS[k][j]):
                one_sum += time_unaveraged_RDFS[k][j]
                count += 1
        
        if count > 0:
            one_sum = one_sum / count
        else:
            one_sum = np.nan
