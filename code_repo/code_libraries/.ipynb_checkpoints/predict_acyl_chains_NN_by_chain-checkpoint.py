...
from code_libraries.voron_tess_w_chol_overlay_helper import _compute_periodic_voronoi
#from voron_tess_w_chol_overlay_helper import *
from code_libraries.voron_tess_w_chol_tiles_helper import _compute_periodic_voronoi_chol_tiles
#from voron_tess_w_chol_tiles_helper import *


class clf_lo_ld_by_chain:
    ...
    processed_acyl_c4 = 0
    processed_acyl_dfs = 0
    processed_scaled_acyl_dfs = 0
    
    processed_chol_coms = 0 
    processed_chol_heads =0
    
    lo_prob_cutoff = 0.5
    prediction_probs = 0
    prediction_labels = 0

    def __init__(self, sys_name, sys_type, red_type, model_type , chain_type ,n_point, start_frame_real, stop_frame_real):
        ...
        if self.red_type != "raw":
            ...
            self.scaler_name = str(self.model_type)+"_"+str(self.chain_type)+"_"+str(self.n_point)+"pt_scaler_"+str(self.red_type)+".sav"
            self.model_name = str(self.model_type)+"_"+str(self.chain_type)+"_"+str(self.n_point)+"pt_model_"+str(self.red_type)+".sav"

        else:

            self.scaler_name = str(self.model_type)+ "_"+str(self.chain_type) + "_raw_scaler.sav"
            self.model_name = str(self.model_type)+ "_"+str(self.chain_type) +"_raw_model.sav"
    ...
    def bilayer_all_data(self):
        ...
        self.box_dimensions = bilayer_init.box_dimensions()
    ...
    def chol_oxy(self):
        ...
        chol_pos_lower , chol_pos_upper = self.bilayer_all_chol_data()
        chol_oxy_lower = chol_pos_lower[: ,: ,  2 , :]
        chol_oxy_upper = chol_pos_upper[: ,:, 2 , :]
        
        return chol_oxy_lower , chol_oxy_upper
    ...
    def chol_oxy_av(self):

        chol_oxy_lower, chol_oxy_upper = self.chol_oxy()

        if self.red_type == "ft_last":

            chol_av_lower = com_ft_last(chol_oxy_lower , self.n_point)
            chol_av_upper = com_ft_last(chol_oxy_upper , self.n_point)

        elif self.red_type == "sav_gol":

            chol_av_lower = com_savgol(chol_oxy_lower , self.n_point)
            chol_av_upper = com_savgol(chol_oxy_upper , self.n_point)

        else:

            chol_av_lower , chol_av_upper = chol_oxy_lower , chol_oxy_upper
    ...
    def process_all_input(self):
        ...
        try:
            ...
            self.processed_chol_heads = self.chol_oxy_av()

        except ValueError:

            self.processed_chol_coms = None , None
            self.processed_chol_heads = None , None

        lower_dopc, upper_dopc = self.bilayer_all_dopc_data()

        dopc_acyl_lower_coms = acyl_chain_one_com(lower_dopc , self.chain_type)
        dopc_acyl_upper_coms = acyl_chain_one_com(upper_dopc , self.chain_type)

        if self.chain_type == "sn1":
            dopc_acyl_lower_c4 = lower_dopc[: , : , 98 , :]
            dopc_acyl_upper_c4 = upper_dopc[: , : , 98 , :]

        else:
            dopc_acyl_lower_c4 = lower_dopc[: , : , 51 , :]
            dopc_acyl_upper_c4 = upper_dopc[: , : , 51 , :]
            
        # dopc_lower_nitro = lower_dopc[: , : , 0 , :]
        # dopc_upper_nitro = upper_dopc[: , : , 0 , :]
        # dopc_lower_phos = lower_dopc[: , : , 19 , :]
        # dopc_upper_phos = upper_dopc[: , : , 19  , :]
        
        dopc_acyl_lower_df = op_height_df(lower_dopc , self.chain_type , self.stop_frame_real , is_pure=True, heights = True)
        dopc_acyl_upper_df = op_height_df(upper_dopc , self.chain_type , self.stop_frame_real , is_pure=True, heights = True)
        ...
        if self.red_type == "ft_last":
            ...
            dopc_acyl_lower_df = convolve_features_valid(dopc_acyl_lower_df , self.n_red , self.stop_frame_real , label_type="pure")
            dopc_acyl_upper_df = convolve_features_valid(dopc_acyl_upper_df , self.n_red , self.stop_frame_real , label_type="pure")
            # dopc_lower_sn2_df = convolve_features_valid(dopc_lower_sn2_df , self.n_red , self.stop_frame_real , label_type="pure")
            # dopc_upper_sn2_df = convolve_features_valid(dopc_upper_sn2_df , self.n_red , self.stop_frame_real , label_type="pure")

            dopc_acyl_lower_coms = com_ft_last(dopc_acyl_lower_coms , self.n_point)
            dopc_acyl_upper_coms = com_ft_last(dopc_acyl_upper_coms , self.n_point)
            dopc_acyl_lower_c4 = com_ft_last(dopc_acyl_lower_c4 , self.n_point)
            dopc_acyl_upper_c4 = com_ft_last(dopc_acyl_upper_c4 , self.n_point)            
            # dopc_lower_phos = com_ft_last(dopc_lower_phos , self.n_point)
            # dopc_upper_phos = com_ft_last(dopc_upper_phos , self.n_point)
            # dopc_lower_nitro = com_ft_last(dopc_lower_nitro , self.n_point)
            # dopc_upper_nitro = com_ft_last(dopc_upper_nitro , self.n_point)

        elif self.red_type == "sav_gol":

            dopc_acyl_lower_df = sav_gol_features(dopc_acyl_lower_df , self.n_point , self.sav_gol_poly_order , self.stop_frame_real , label_type="pure")
            dopc_acyl_upper_df = sav_gol_features(dopc_acyl_upper_df , self.n_point , self.sav_gol_poly_order , self.stop_frame_real , label_type="pure")
            # dopc_lower_sn2_df = sav_gol_features(dopc_lower_sn2_df , self.n_point , self.sav_gol_poly_order , self.stop_frame_real , label_type="pure")
            # dopc_upper_sn2_df = sav_gol_features(dopc_upper_sn2_df , self.n_point , self.sav_gol_poly_order , self.stop_frame_real , label_type="pure")


            dopc_acyl_lower_coms = com_savgol(dopc_acyl_lower_coms , self.n_point)
            dopc_acyl_upper_coms = com_savgol(dopc_acyl_upper_coms , self.n_point)
            dopc_acyl_lower_c4 = com_savgol(dopc_acyl_lower_c4 , self.n_point)
            dopc_acyl_upper_c4 = com_savgol(dopc_acyl_upper_c4 , self.n_point)               
            # dopc_lower_phos = com_ft_last(dopc_lower_phos , self.n_point)
            # dopc_upper_phos = com_savgol(dopc_upper_phos , self.n_point)
            # dopc_lower_nitro = com_savgol(dopc_lower_nitro , self.n_point)
            # dopc_upper_nitro = com_savgol(dopc_upper_nitro , self.n_point)
        ...
        dopc_acyl_lower_df = dopc_acyl_lower_df.iloc[: , :-1]
        dopc_acyl_upper_df = dopc_acyl_upper_df.iloc[: , :-1]

        

        self.processed_dfs = (dopc_acyl_lower_df , dopc_acyl_upper_df)
        self.processed_acyl_coms = (dopc_acyl_lower_coms , dopc_acyl_upper_coms)
        self.processed_acyl_c4 = (dopc_acyl_lower_c4 , dopc_acyl_upper_c4)
        # self.processed_dopc_phos = (dopc_lower_phos , dopc_upper_phos)
        # self.processed_dopc_nitro = (dopc_lower_nitro , dopc_upper_nitro)
    ...
    def predict_average_probability(self , acyl_or_head):

        predictions_lower, predictions_upper = self.prediction_probs

        if acyl_or_head == "acyl":
            
            lower_leaflet_array, upper_leaflet_array = self.processed_acyl_coms

        else:

            lower_leaflet_array, upper_leaflet_array = self.processed_dopc_phos
    ...
    def plot_predicted_average_all_frames_simple(self , acyl_or_head, sterol_or_head, title , plot_type="2d",figure_size = [12,8]):


        coord_av_lower , prob_av_lower , coord_av_upper, prob_av_upper = self.predict_average_probability(acyl_or_head)
        ...
        if plot_type != "2d":
            ...
            try:
                ...
                if sterol_or_head == "sterol":
                    
                    chol_lower , chol_upper = self.processed_chol_coms

                else:

                    chol_lower , chol_upper = self.processed_chol_heads
    ...
    #         self, acyl_or_head , sterol_or_head,
    ...
    #         self.predict_average_probability(acyl_or_head)
    ...
    #         if handles:
    #             #ax.legend(loc="upper right", fontsize=8, frameon=False) ###uncomment to reenable legends
...
### Voronoi Tesselation with Chol tiles
    ...
    def plot_predicted_average_voronoi_periodic_with_chol(self, acyl_or_head , sterol_or_head, conc, title="Periodic Voronoi Lo probability (with CHOL)", leaflet="both", xy_lim = 10, figure_size=(12, 6), cmap="viridis"):
        """
        ...
        def _plot_leaflet(ax, coord_av, prob_av, chol_av, label):
            ...
            ax.set_xlim(0.0, xy_lim)
            ax.set_ylim(0.0, xy_lim)
            ax.set_aspect("equal", "box")
        ...
        if mappable is not None:
            cbar = fig.colorbar(mappable, ax=fig.axes, shrink=0.4)
            cbar.set_label(fr"Lo-type Probability , $\chi_C$ = {conc:.2f}")
