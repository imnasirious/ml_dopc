...
class clf_acyl_blk_av:
    
    main_dir = "/Users/nasirsiddiqui_2/Desktop/ML_research/"
    dataframe_dir = os.path.join(main_dir ,"heavy_data_NO_GIT/dataframes")
    model_scaler_dir = os.path.join(main_dir , "bilayer_classification_project")
    
    sn1_indices = [41, 42 , 43]
    sn1_indices.extend(i for i in range(91 ,134))
    sn1_indices_carbon_only = np.array([42 , 92, 95, 98, 101 , 104,107, 110,  112, 114 , 117 , 120 , 123, 126, 129,   132])


    sn2_indices = [32, 33 , 34]
    sn2_indices.extend(i for i in range(44 ,91))
    sn2_indices_carbon_only = np.array([33 , 45 , 48 , 51 , 54 , 57 , 60 , 63 , 65 , 67 , 70 , 73 , 76 , 79 , 82 , 85])
    ...
    def __init__(self ,sys_name , sys_type ,n_red, start_frame_real , stop_frame_real):
    ...
    def bilayer_all_data(self):
        ...
        bilayer_universe = Bilayer(self.sys_name ,self.sys_type , self.start_frame_real , self.stop_frame_real)
        bilayer_leaflets = bilayer_universe.cleave_leaflets()
        
        upper_leaflet_all = bilayer_leaflets[0]
        lower_leaflet_all = bilayer_leaflets[1]
    ...
    def cholesterol_blk_av(self):
        chol_lower , chol_upper = self.bilayer_all_chol_data()
        chol_lower_convolved = block_av_trajectory(chol_lower , self.stop_frame_real, self.n_red)
        #chol_lower_convolved = np.swapaxes(chol_lower_convolved , 0 , 1)
        chol_upper_convolved = block_av_trajectory(chol_upper , self.stop_frame_real, self.n_red)
    ...
    def process_all_input(self):
        ...
        dopc_lower_all_convolved = block_av_trajectory(lower_dopc , self.stop_frame_real , self.n_red)
        #dopc_lower_all_convolved = np.swapaxes(dopc_lower_all_convolved , 0 , 1)
        
        
        dopc_upper_all_convolved = block_av_trajectory(upper_dopc , self.stop_frame_real , self.n_red)
        ...
        self.processed_dfs = lower_sn1_sn2_combined.iloc[: , :-1] , upper_sn1_sn2_combined.iloc[: , :-1]

        scaler_name = str("svc_") + str(self.n_red) + str("_scaler_blk_av.sav")
        scaler_path = os.path.join(self.model_scaler_dir , scaler_name)
        scaler = pickle.load(open(scaler_path , 'rb'))
        
        unscaled_lower , unscaled_upper = self.processed_dfs
        scaled_lower , scaled_upper = scaler.transform(unscaled_lower) , scaler.transform(unscaled_upper)
    ...
    def classify(self , probability = False):
        ...
        model_name = str("svc_") + str(self.n_red) + str("_model_blk_av.sav")
        model_path = os.path.join(self.model_scaler_dir , model_name)
